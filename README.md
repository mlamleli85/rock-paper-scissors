# Rock, Paper, Scissors

## Description

The Rock, Paper, Scissors is a game many of would have played growing up. This is now being offered on an online platform where the user can challenge the computer to see who comes out on top. The user selects their "weapon" and the computers' weapon is randomly selected.

The result of each round is then displayed on the screen with the scores updating automatically to track who is ahead.

![alt text] responsiveness screenshot.png

## Features

### Header

- At the top of the page is the title of the game.

### Play Area

- Below that you find the three buttons from which the user can choose. Icons taken from Font Awesome assist the user with a visual to make it easier to know what they are selecting.
- These icons are highlighted when the user moves the cursor over them.

### Results

- Once the user has made their selection, this will be displayed on the left side of the screen.
- The computer's selection is randomly selected and displayed opposite the player's selection on the left of the screen.
- A message is then displayed on the screen showing the winner of the round.
- The score of the user and computer are then displayed on the corresponding side of the screen. This is updated automatically to keep track of the scores and is always visible.
  ![image alt](https://github.com/mlamleli85/rock-paper-scissors/blob/67dfa188bec1343a347ab4d5664054bb7e156c8f/Screenshot%202026-03-09%20at%2012.55.40.png)

### Testing

- The website was tested on Google Chrome, Safari and Firefox. It worked without any issues on all three browsers.
- Using DevTools, I tested the website's responsiveness and functionality on small to extra-large screen sizes.

### Validation Tests

- No errors were detected using the W3C Markup Validation Sevice (HTML).
- No errors were detected using the W3C CSS Validation Sevice (CSS).
- Using the Lighthouse feature in DevTools, I tested the website's accessiblity.

### Bugs

- There are no unfixed bugs on the website.

## Deployment

- The website was deployed to GitHub pages.
- This is the live link to the website: (https://mlamleli85.github.io/rock-paper-scissors/)

## Credits

- To start my JavaScript code, I used the code to declare constants for DOM elements, which were shown in the Portfolio Project Scope video.

### Media

- The images used on the website were generated using Adobe Firefly (https://www.firefly.adobe.com/)
